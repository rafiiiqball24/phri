  ...
  <ClientOnly>
    <section class="page container">
      ...
      <div v-if="showDebug" class="dbg">
        ...
        <button class="dbg__btn" :class="{ on: dbg.loading }" @click="toggleLoading">{{ dbg.loading ? 'Matikan' :
          'Nyalakan' }} Skeleton</button>
        <button class="dbg__btn" :class="{ on: dbg.empty }" @click="toggleEmpty">{{ dbg.empty ? 'Isi Lagi' : 'Kosongkan'
        }} Keranjang</button>
      </div>
      ...
      <div v-else class="cols">
        ...
        <SummaryBox v-if="!dbg.loading" title="Ringkasan Pembelian"
          :count-label="`${selectedItems.length} Item dipilih`" :lines="[
...
onMounted(() => {
  ...
  prevKeys.value = new Set(displayedItems.value.map((it) => it.key))
})
...
watch(displayedItems, (arr) => {
  ...
  for (const it of arr) if (!prevKeys.value.has(it.key) && !selected.value.has(it.key)) selected.value.add(it.key)
  prevKeys.value = keysNow
  persistSelection()
}, { deep: true })

function isChecked(key: string) { return selected.value.has(key) }
function toggleOne(key: string) { if (selected.value.has(key)) selected.value.delete(key); else selected.value.add(key); persistSelection() }
const allChecked = computed(() => displayedItems.value.length > 0 && displayedItems.value.every((it) => selected.value.has(it.key)))
function toggleAll() { if (allChecked.value) selected.value.clear(); else for (const it of displayedItems.value) selected.value.add(it.key); persistSelection() }
function persistSelection() { try { localStorage.setItem('phri_cart_selected_keys', JSON.stringify([...selected.value])) } catch { } }

const selectedItems = computed<CartItem[]>(() => displayedItems.value.filter((it) => selected.value.has(it.key)))
const selectedSubtotal = computed(() => selectedItems.value.reduce((s, it) => s + Number(it.price) * Number(it.qty || 0), 0))
const selectedTotalQty = computed(() => selectedItems.value.reduce((s, it) => s + (Number(it.qty) || 0), 0))
...
function goCheckout() {
  ...
  ensureSession()
  try { localStorage.setItem('phri_checkout_selection', JSON.stringify(selectedItems.value.map((it) => it.key))) } catch { }
...
function normalizeImages(p: ProductApi): string[] {
  ...
  const imgs = arr.map((it: any) => (typeof it === 'string' ? it : it?.url || it?.path || it?.src)).filter(Boolean).map((u: string) => asset(u))
...
function mapToCard(p: ProductApi) {
  ...
  return { to: { path: '/detail-product', query: { slug: p.slug } }, name: p.name, image: normalizeImages(p)[0] || asset(p.thumbnail) || '', price: Number(p.price ?? 0), tags, soldOut: Number(p.quantity ?? 0) <= 0 }
}
async function fetchRecommendations() {
  try {
    const res: any = await $fetch('/product', { baseURL, headers, params: { page: 1 } })
    const arr: ProductApi[] = res?.data?.products?.data || res?.data?.products || res?.data || []
...
async function syncCartStocks() {
  try {
    ...
    const results = await Promise.allSettled(uniqueIds.map(async (id) => {
      const res: any = await $fetch(`/product/${id}`, { baseURL, headers })
      const root = res?.data?.product || res?.data || res
    ...
    for (const it of arr) {
      ...
      const max = (it as any).stock ?? Infinity
      if (typeof it.qty === 'number' && it.qty > max) { (updateQty as any)(it.key, max); it.qty = max }
    }
  } catch { }
}
